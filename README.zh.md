...
开头的 emoji 是**活跃度标记**，由 `scripts/update-metadata.sh` 自动刷新。新条目先用 🟢，下次脚本运行时会按仓库实际活跃度修正。
...
- `scripts/update-metadata.sh` — 通过 GitHub API 刷新活跃度标记（`--check` 仅报告，`--update` 改写 README）
